...
export default function CandidateCard({ candidate }: { candidate: Candidate }) {
  ...
  return (
    <Link
      ...
      className="group block border-b border-[#1e293b] py-8 transition-all duration-300 hover:translate-x-1 md:py-10 last:border-b-0"
    >
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_220px] lg:gap-10">
        <div className="max-w-3xl">
          <div className="mb-4 flex flex-col gap-3">
            <span className="text-xs uppercase tracking-[0.22em] text-[#64748b]">
              {candidate.race} · {candidate.location}
            </span>
            <h3 className="font-serif text-[2rem] leading-tight text-[#faf7f2] transition-colors group-hover:text-[#d97706] md:text-[2.2rem]">
              {candidate.name}
            </h3>
          </div>
          <p className="mb-4 text-sm font-medium text-[#d97706]">
            &ldquo;{candidate.tagline}&rdquo;
          </p>
          <p className="mb-5 max-w-2xl text-sm leading-7 text-[#94a3b8]">
            {candidate.summary}
          </p>
          {latestDecision && (
            <div className="grid gap-3 border-t border-[#1e293b] pt-5 text-sm md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center md:gap-4 max-w-2xl">
        ...
        <div className="rounded-[24px] border border-[#223046] bg-[#111827]/24 p-5 lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:pt-1 lg:text-right">
