  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#1e293b]/80 bg-[#0f172a]/88 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 md:px-12 lg:px-24">
        <Link
          href="/"
          className="flex items-center gap-3 text-[#faf7f2] transition-colors hover:text-white"
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#253247] text-[#cbd5e1] transition-colors hover:border-[#d97706] hover:text-[#faf7f2] md:hidden"
        >
          ...
          <nav className="mx-auto flex max-w-7xl flex-col gap-1 px-6 py-4">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setOpen(false)}
                className="rounded-2xl px-4 py-3 text-base text-[#cbd5e1] transition-colors hover:bg-[#111a2b] hover:text-[#faf7f2]"
              >
                {item.label}
              </Link>
            ))}
